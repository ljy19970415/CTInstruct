    ...
    def train_one_iter(self,task_name,criterion=None,it=0):
        ...
        if "segmentation" in task_name:
            ...
            with autocast(dtype=torch.float16):
                seg_logits=self.model.segmentation_forward(image, mark, task_name,patch_num, chosen_label_idx, self.model.local_rank) # b cls_num H W D
            ...
            with autocast(dtype=torch.float16):
                cls_logits, cls_label = self.model.classification_forwad(image,mark,cls_label,task_name,patch_num, self.model.local_rank,self.flag_2D_dict[task_name])
            ...
            patch_num = batch['patch_num']
            with autocast(dtype=torch.float16):
                loss = self.model.imagetext_forward(image, text, mark, patch_num, self.model.local_rank)
