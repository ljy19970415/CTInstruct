...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: torch.FloatTensor, key: torch.FloatTensor, value: torch.FloatTensor,
        ...
        if mask is not None:
            ...
            scores = scores.masked_fill(mask.eq(0), _MASKING_VALUE)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.FloatTensor, key: torch.FloatTensor, value: torch.FloatTensor,
        ...
        # x: B x H x L x D_v
        x, attn, scores = self.sdpa(query, key, value, mask=mask, dropout=self.dropout)

        # 3) "Concat" using a view and apply a final linear.
        x = x.transpose(1, 2).contiguous().view(batch_size, -1, self.h * self.d_k)
        return self.linears[-1](x), attn, scores