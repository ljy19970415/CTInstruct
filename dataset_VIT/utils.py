...
def rgb2gray(rgb):
    ...
    if len(rgb.shape) == 3:
        ...
        dic['rgb_shape2'] = list(gray.shape) 
        gray = gray.unsqueeze(-1)
        dic['rgb_shape3'] = list(gray.shape)
...
def load_from_bucket(url):
    monai_loader = monai.transforms.Compose(
        [
            # monai.transforms.Orientationd(axcodes="RAS", keys=['image']),
...
def resize_and_pad_image(image, target, target_resolution, flag_2D=False):
    ...
    if target is not None:
        ...
        dic['target3'] = {'min':str(np.min(new_target)),'max':str(np.max(new_target))}
    else:
        new_target = None
